...
I know i am not testing on a production server, anyway this HW mimic very closely a standard server slice offered by common [hosting providers](https://www.digitalocean.com), so it can serve as a common comparison basis.  
Using Vagrant also allows decoupling the server where i run the load tool from the one running the tested application, thus preventing unreliable results.
...
```
wrk -t 2 -c 150 -d30s --timeout 2000 http://192.168.33.22:9292
```
...
##### Considerations
While it is true that Node.js suffers JavaScript single threaded nature, it has proven to have very good performance indeed.  
...
## Conclusions
To layout my personal ranking i do not only focus on recorded performance, but also on simplicity of the program, dependencies footprint and setup times.

### 1. GO
Balancing excellent speed, consistency and ease of use GO is the winner for me, not only by the benchmarks. 
